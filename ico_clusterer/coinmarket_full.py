...
#########
#Downloading Coinmarketcap.com data - all crypto names
data2 = urllib2.urlopen('https://coinmarketcap.com/all/views/all/')
	
	
token =[]		
for line in data2: # files are iterable
	line = line.replace(' <span class="currency-symbol"><a href="/currencies/','<span class="currency-symbol"><a href="/currencies/')
	line = line.replace('  <span class="currency-symbol"><a href="/currencies/','<span class="currency-symbol"><a href="/currencies/')
	
	if '<span class="currency-symbol"><a href="/currencies/' in line:
		line = line.replace('/">','00000000000000000000000000000000000')
		token.append(line[51:85].split("0")[0])
		#print line[51:85].split("0")[0]
...
#data = urllib2.urlopen('https://coinmarketcap.com/currencies/'+currency+'/historical-data/?start=20130428&end='+today)
		data = urllib2.urlopen('https://coinmarketcap.com/currencies/'+item+'/historical-data/?start=20130428&end='+today)
...
#following code saves output to file
		if export:
			with open('coin_data/series_'+item+'.dat','w') as f1:
