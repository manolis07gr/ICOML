...
with open('outdata/ico_data_full.csv', 'w') as csvfile1, open('outdata/ico_data_reduced.csv', 'w') as csvfile2, open('outdata/ico_data_reduced_wratings.csv', 'w') as csvfile3:
    ...
    for i in range(0,len(coin)):
        ...
        try:
            ...
            industry = func_industry(industry_all[0])  # WAS industry_all[len(industry_all)-1]
        ...
        #5A) Check if data availabe from icobench or icodrops. If yes adopt this money raised value
        if (raised_all[0] != 'N/A') or (raised_all[1] != 'N/A'):
            raised = raised_all[1]

        #5B) In the opposite case, proceed as before
        if (raised_all[0] == 'N/A') or (raised_all[1] == 'N/A'):
        ...
        #5F) If values are not the same then adopt the value that appears most times. If failure adopt first element.
            try:
                ...
                raised = raised_all[1]  # WAS raised_all[0]
        ...
        try:
            ...
            hardcap = round(hardcap_all[1],0)  # WAS hardcap_all[0]
