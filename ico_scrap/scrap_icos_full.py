...
#Get unlisted ICOs
with open("outdata/ico_unlisted.csv") as fg:
    reader = csv.reader(fg)
    unlisted_cryptos = [rg for rg in reader]

crypto_data = np.concatenate((crypto_data, unlisted_cryptos))

#Removal of duplicate rows/entries
l = -1
crypto_data2 = []
for i in range(0,len(crypto_data)):
    temp = crypto_data[i][0]
    k = 0
...
for i in range(0,len(crypto_data)):
    ...
    if data_vec[1][2] != 'N/A':
        ...
        full_data[j] = data_vec[0]
        target_data[j] = data_vec[1]
...
#USUALLY COMMENT OUT ABOVE

with open('outdata/ico_data_full.txt', 'w') as f:
...
today_day_o = datetime.strptime(today_day, '%Y%m%d')

#GOOGLE news search window below has been temporarily disabled - not important info/covered by Alexa data
"""
...
#The last step above produces the final REDUCED dataset that includes coindesk AND unlisted/current ICO entries and for
#no CMC data issues values of -100
...
# Now output the dataset that is complete for: team, price, hardcap, telegram, twitter, N_daily_views, N_daily_time, success
# for ICOs that have closed (end date < current date). This is the dataset that will be used for ICO rating and ICO clustering
...
with open('outdata/ico_data_rate_cluster.csv', 'w') as csvfile_bb:
    csvfile_bb.write(columnTitles_rateCluster)
    writer=csv.writer(csvfile_bb, delimiter=',')

    for i in range(0,len(target_data)):
        end_date_a = target_data[i][2].replace(" ","")
        end_day_o = datetime.strptime(end_date_a, '%d%b%Y')
        if ('N/A' not in target_data[i][0:3]) and ('N/A' not in target_data[i][7:17]) and (today_day_o > end_day_o):
            rate_data = [target_data[i][0],target_data[i][1],target_data[i][2],target_data[i][3],target_data[i][4],target_data[i][5],target_data[i][6],target_data[i][7],target_data[i][8],target_data[i][9],target_data[i][10],target_data[i][11],target_data[i][12],target_data[i][13],target_data[i][14],target_data[i][15],target_data[i][16],target_data[i][17],target_data[i][18],target_data[i][19],target_data[i][20],target_data[i][21],target_data[i][22],target_data[i][23],target_data[i][24],target_data[i][25],target_data[i][26],target_data[i][27],target_data[i][28],target_data[i][29]]
